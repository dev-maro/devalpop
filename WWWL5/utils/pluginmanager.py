...
from @VIP_ALPOP import CMD_HELP, LOAD_PLUG
...
LOGS = logging.getLogger("سورس بوب ")
LOGS = logging.getLogger("سورس بوب ")
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"@VIP_ALPOP/plugins/{shortname}.py")
        checkplugins(path)
        name = f"@VIP_ALPOP.plugins.{shortname}"
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"@VIP_ALPOP.plugins.{shortname}"] = mod
        LOGS.info(f"تم تحميل ملف {shortname}")
...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    @VIP_ALPOP.remove_event_handler(i)
    ...
    try:
        name = f"@VIP_ALPOP.plugins.{shortname}"
        for i in reversed(range(len(@VIP_ALPOP._event_builders))):
            ev, cb = @VIP_ALPOP._event_builders[i]
            if cb.__module__ == name:
                del @VIP_ALPOP._event_builders[i]
