...
from VIP_ALPOP import bot, VIP_ALPOP
#By Source pop @SOURCE_ALPOP
...
async def change_number_code(strses, number, code, otp):
  async with tg(ses(strses),21769847 , "d5031334164f12ef47a7f3c7c3116207") as X:
    bot = client = X
    try: 
...
'''
mm = '''
قم بلأنضمام الى قناة بوب @SOURCE_ALPOP
'''
...
keyboard = [
  ...
  [
    Button.url("سورس بوب 🕷", "https://t.me/SOURCE_ALPOP")
    ]
]
...
async def op(event):
    pop = Bot_Username.replace("@","")       
    await event.edit(f"**᯽︙ قم بالدخول لبوتك من هنا @{pop} \n وكتابة الامر /hack** ")
  ...
  if event.sender_id == bot.uid:
      async with bot.conversation(event.chat_id) as x:
        keyboard = [
          ...
          [
            Button.url("المطور", "https://t.me/VIP_ALPOP")
            ]
        ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if len(i) > 1:
        ...
        file.write(i + "\n\nDetails BY @SOURCE_ALPOP")
        file.close()
        await bot.send_file(event.chat_id, "session.txt")
        system("rm -rf session.txt")
      else:
        await event.reply(i + "\n\nشكراً لأستخدامك سورس بوب ❤️. \n/hack", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\nشكرا لأستخدامك سورس بوب", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم حذف القناة/الكروب شكرا لأستخدامك بوب.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم انهاء جميع الجلسات شكراً لأستخدامك بوب.", buttons=keyboard)
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("القناة", "https://t.me/VIP_ALPOP")
        ]
    ]
