...
@M_4_R_0.ar_cmd(pattern="الصيد")
async def _(event):
    await event.edit(
        """
⩹⌯⊷━♢ ⦓ 𝗧𝗘𝗜𝗘𝗧𝗛𝗢𝗡 𝗠𝗔𝗥𝗢 ⦔ ♢━⊶⌯⩺
أوامر الصيد الخاصة بسورس مارو  : 
...
⩹⌯⊷━♢ ⦓ 𝗧𝗘𝗜𝗘𝗧𝗛𝗢𝗡 𝗠𝗔𝗥𝗢 ⦔ ♢━⊶⌯⩺
⌯︙ [┈┉━｢ 𝙎𝙊𝙐𝙍𝘾𝙀 𝙈𝘼𝙍𝙊 🕷 ｣━┅┈](t.me/M_4_R_0)

"""
    )


@M_4_R_0.ar_cmd(pattern="صيد (.*)")
async def hunterusername(event):
    choice = str(event.pattern_match.group(1))
    try:
        ch = await M_4_R_0(
            functions.channels.CreateChannelRequest(
                title="MARO HUNTER - صيد مارو ",
                about="This channel to hunt username by - @M_4_R_0 ",
            )
        )
...
async def _(event):
    ...
    isclaim.clear()
    isclaim.append("off")
    await M_4_R_0.send_message(event.chat_id, "**- تم الانتهاء من التثبيت بنجاح**")


@M_4_R_0.ar_cmd(pattern="حالة الصيد")
async def _(event):
    if "on" in isclaim:
...
@M_4_R_0.ar_cmd(pattern="حالة التثبيت")
async def _(event):
    if "on" in isauto:
