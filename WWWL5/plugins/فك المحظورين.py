...
async def unblock_users(M_4_R_0):
    @M_4_R_0.on(events.NewMessage(outgoing=True, pattern=".فك المحظورين"))
    async def _(event):
        list = await M_4_R_0(GetBlockedRequest(offset=0, limit=1000000))
        if len(list.blocked) == 0:
            razan = await event.edit(f"- لم تقم بحظر اي شخص اصلا .")
        else:
            unblocked_count = 1
            for user in list.blocked:
                UnBlock = await M_4_R_0(UnblockRequest(id=int(user.peer_id.user_id)))
                unblocked_count += 1
...
loop.create_task(unblock_users(M_4_R_0))
