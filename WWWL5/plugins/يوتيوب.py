...
@M_4_R_O.ar_cmd(pattern="تحميل صوتي(?:\s|$)([\s\S]*)")
async def download_audio(event):
    ...
    for url in urls:
        try:
            ...
            vid_data = {"title": url, "uploader": "M_4_R_O", "formats": []}
        startTime = time()
...
@M_4_R_O.ar_cmd(pattern="تحميل فيديو(?:\s|$)([\s\S]*)")
...
@M_4_R_O.ar_cmd(pattern="انستا(?: |$)([\s\S]*)")
async def insta_dl(event):
    ...
    async with event.client.conversation(v1) as conv:
        try:
            ...
            await M_4_R_O(unblock("IgGramBot"))
            v1_flag = await conv.send_message("/start")
        ...
        async with event.client.conversation(v2) as conv:
            try:
                ...
                await M_4_R_O(unblock("videomaniacbot"))
                v2_flag = await conv.send_message("/start")
            await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
...
@M_4_R_O.ar_cmd(pattern="نتائج(?: |$)(\d*)? ?([\s\S]*)")
