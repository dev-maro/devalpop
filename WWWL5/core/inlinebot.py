...
CATLOGO = "https://telegra.ph/file/96e2761d3aabfdf79d313.jpg"
...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**M_4_R_O"):
            buttons = [
                (
                    Button.inline("Stats", data="stats"),
                    Button.url("DEV", "https://t.me/M_4_R_0"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./M_4_R_O", "troll.txt")
            try:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./M_4_R_O", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./M_4_R_O", "hide.txt")
            try:
            ...
            result = builder.article(
                title="M_4_R_O Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب مارو **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/M_4_R_0"),
                Button.url(
                    "المطور",
                    "t.me/M_4_R_O",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل مارو من تنصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[𝗠𝗔𝗥𝗢 𝙐𝙎𝙀𝙍𝘽𝙊𝙏 🧸♥](https://t.me/M_4_R_0)",
...
@M_4_R_O.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@M_4_R_O.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@M_4_R_O.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    category = str(event.pattern_match.group(1).decode("M_4_R_0"))
...
@check_owner
async def on_plug_in_callback_query_handler(event):
    mtype = str(event.pattern_match.group(1).decode("M_4_R_0"))
    category = str(event.pattern_match.group(2).decode("M_4_R_0"))
    pgno = int(event.pattern_match.group(3).decode("M_4_R_0"))
    if mtype == "plugin":
        ...
        category_plugins = str(event.pattern_match.group(4).decode("M_4_R_0"))
        category_pgno = int(event.pattern_match.group(5).decode("M_4_R_0"))
...
@M_4_R_O.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
...
async def on_plug_in_callback_query_handler(event):
    category = str(event.pattern_match.group(1).decode("M_4_R_0"))
    current_page_number = int(event.data_match.group(2).decode("M_4_R_0"))
    htype = str(event.pattern_match.group(3).decode("M_4_R_0"))
    if htype == "plugin":
...
async def on_plug_in_callback_query_handler(event):
    ...
    if category_plugins:
        category_plugins = str(category_plugins.decode("M_4_R_0"))
    category_pgno = event.pattern_match.group(5)
    if category_pgno:
        category_pgno = int(category_pgno.decode("M_4_R_0"))
    if htype == "plugin":
        buttons = paginate_help(current_page_number + 1, GRP_INFO[category], category)
    else:
        buttons = paginate_help(
...
@check_owner
async def on_plug_in_callback_query_handler(event):
    cmd = str(event.pattern_match.group(1).decode("M_4_R_0"))
    category = str(event.pattern_match.group(2).decode("M_4_R_0"))
    pgno = int(event.pattern_match.group(3).decode("M_4_R_0"))
    category_plugins = str(event.pattern_match.group(4).decode("M_4_R_0"))
    category_pgno = int(event.pattern_match.group(5).decode("M_4_R_0"))
